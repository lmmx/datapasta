...
def guess_separator(lines: List[str], candidates: Optional[List[str]] = None) -> str:
...
def parse_table(text: str, sep: Optional[str] = None, max_rows: int = 200) -> Dict[str, Any]:
    ...
    # Determine if first row is a header by comparing types with the rest
    has_header = False
    ...
    if data_rows:
        ...
        for i in range(check_rows):
            if i < len(data_rows):
                ...
                if len(row) == len(first_row):
                    rest_rows_types.append([not is_numeric(val) for val in row[:len(first_row)]])
        ...
        if rest_rows_types:
            ...
            has_header = different_count >= len(first_row) / 2
